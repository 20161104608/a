...
int main(int argc, const char * argv[]) {
    ...
    int n;
    fp1=fopen("//Users//s20161104608//Desktop//123//gps.txt","w+");
    fp2=fopen("//Users//s20161104608//Desktop//123//GPS170510.log","r");
    fp3=fopen("//Users//s20161104608//Desktop//123//gps.csv","w+");
    if(fp2==NULL)
    {
    ...
    {
        while(fscanf(fp2,"$GPRMC,%6s,%c,%8s,%c,%9s,%c,%lf,%lf,%6s,,%3s\n$GPGGA,%6s,%lf,%c,%lf,%c,%d,%2s,,%4s,M,,M,,%3s\n",time,&gps,latitude,&latitude1,longitude,&longitude1,&v,&course,year,s,Time,&Latitude,&Latitude1,&Longitude,&Longitude1,&n,moon,altitude,S)!=EOF)
        {
            if(gps=='A')
            {
                fprintf(fp1,"当前是%c%c年%c%c月%c%c日%c%c点%c%c分%c%c秒\n",year[4],year[5],year[2],year[3],year[0],year[1],time[0],time[1]+8,time[2],time[3],time[4],time[5]);
                printf("当前是%c%c年%c%c月%c%c日%c%c点%c%c分%c%c秒\n",year[4],year[5],year[2],year[3],year[0],year[1],time[0],time[1]+8,time[2],time[3],time[4],time[5]);
                fprintf(fp3,"%c%c年%c%c月%c%c日,%c%c点%c%c分%c%c秒,",year[4],year[5],year[2],year[3],year[0],year[1],time[0],time[1]+8,time[2],time[3],time[4],time[5]);
                if(latitude1=='N')
                {
